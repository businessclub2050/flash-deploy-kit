...
phase_create() {
    ...
    echo "                 dns-nameservers 10.42.0.1"
    echo "         EOF"
    echo "         systemctl restart networking"
    echo ""
    echo "  6. Copy and run the provisioning script:"
    echo "       scp /root/provision-nemoclaw.sh root@${VMIP}:/root/"
    echo "       ssh root@${VMIP} 'chmod +x /root/provision-nemoclaw.sh && /root/provision-nemoclaw.sh'"
    echo ""
    echo "  7. After provisioning, apply firewall:"
    echo "       ./setup-nemoclaw-vm.sh firewall"
    echo ""
}
...
[RULES]
# Block VM 101 from reaching the hypervisor (hypervisor) — all ports
OUT DROP -dest ${HYPERVISOR_IP} -log nolog
# Allow the hypervisor to SSH into VM 101 for management
...
EOF
    ...
    echo "    - Everything else: ACCEPT (internet, Ollama, DNS)"
    echo ""
    ...
    echo "    # Expected: nemoclaw (ALLOWED)"
    echo ""
}
